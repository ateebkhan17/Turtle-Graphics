...
CmdNode *loadCommands_pro(const char *fileName) {
  CmdNode *head = NULL;
  CmdNode *last = NULL; // Pointer to the last created node outside of a loop.
  CmdNode *forN = NULL;
  CmdNode *temp = NULL;
  ...
  while (fgets(&buffer[0], 1024, f) != NULL) {
    ...
    if (last == NULL) head = temp; // If the last node is NULL, then this is the head of the list.
    
    if (temp != NULL && forN != NULL) insertCommandLoop(forN, temp);
    else if (temp != NULL) last = insertCommand(last, temp);  // Use the last node added to the list as the head.
    else if (forN != NULL) last = insertCommand(last, forN); // Use the last node added to the list as the head.
    
    if (NULL == forN) last = temp; // Update the last node only if not inside a loop.
  }
  fclose(f);
  return head;
}
